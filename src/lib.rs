...
pub use accelerometer::{
    error,
    vector::{F32x3, I16x3, I32x3},
    Accelerometer, Error, RawAccelerometer, Vector,
};
use core::ops::Index;
use micromath::generic_array::{arr, typenum::U3, GenericArray};

pub use fifos::{AccelGyroPacketBigEndian, FifoConfig, FifoMode, Watermark};
// use interrupts::{Interrupt1, Interrupt2};
// use wakeups::{WakeUp};
pub use register::{
    AccelerometerMode, Bank, DataRate, GyroMode, InternalFreqFinetuned, Mode, Range, Register,
    Register_1, Register_3, Timestamp,
};
use register::{
    DEVICE_ID,
    MASK_ACCEL_MODE,
    ...
    CS: OutputPin<Error = PinError>,
{
    pub fn new(spi: SPI, cs: CS, config: &Config) -> Result<Self, DriverError> {
        ...
        if id != DEVICE_ID {
            return Err(DriverError::WrongID);
        }
    ...
    fn switch_reg_bank(&mut self, bank: Bank) {
        self.modify_register(Register::REG_BANK_SEL.addr(), MASK_BANK_SEL, bank.bits())
            .unwrap();
    }

    pub fn set_pull_up_miso(&mut self, mode: u8) {
        self.switch_reg_bank(Bank::Bank3);
        self.modify_register(Register_3::PU_PD_CONFIG2.addr(), PIN1_PU_EN, mode)
            .unwrap();
    ...
    pub fn set_acc_mode(&mut self, mode: AccelerometerMode) {
        self.modify_register(Register::PWR_MGMT0.addr(), MASK_ACCEL_MODE, mode.bits())
            .unwrap();
    ...
    pub fn set_gyro_mode(&mut self, mode: GyroMode) {
        self.modify_register(Register::PWR_MGMT0.addr(), MASK_GYRO_MODE, mode.bits())
            .unwrap();
        cortex_m::asm::delay(12800); // wait 200us
    }
    ...
    pub fn reset(&mut self) {
        self.modify_register(Register::DEVICE_CONFIG.addr(), SOFT_RESET_CONFIG, 0b1)
            .unwrap();
    }
    ...
    pub fn set_data_rate(&mut self, odr: DataRate) {
        ...
        self.modify_register(Register::ACCEL_CONFIG0.addr(), MASK_ACCEL_ODR, odr.bits())
            .unwrap();
        self.modify_register(Register::GYRO_CONFIG0.addr(), MASK_GYRO_ODR, odr.bits())
            .unwrap();
    }
    ...
    pub fn get_timestamp(&mut self) -> Timestamp {
        self.modify_register(Register::SIGNAL_PATH_RESET.addr(), TMST_STROBE, 0x1)
            .unwrap();
    ...
    pub fn set_timestamp_en(&mut self, state: bool) {
        self.modify_register(Register::TMST_CONFIG.addr(), TMST_EN, state as u8)
            .unwrap();
        self.modify_register(Register::TMST_CONFIG.addr(), TMST_TO_REGS_EN, state as u8)
            .unwrap();
    }
    ...
    E: Debug,
{
    ...
    fn accel_norm(&mut self) -> Result<F32x3, Error<Self::Error>> {
        ...
        let norm_values: F32x3 = F32x3::new(
            (raw_values[0] as f64 * lsb_as_mg / 1000.0) as f32, // from G to mG
...
impl Vector for TimestampedAcceleration {
        ...
        I: IntoIterator<Item = Self::Component>,
    {
        ...
        debug_assert!(
            iter.next().is_none(),
            "too many items in 3-axis component slice"
        );
